...
#	OE version info
	...
	OE_NAME=`cd ${AREADELTASAT_BASE} && git submodule | grep "meta-openembedded" | cut -d '(' -f 2 | cut -d ')' -f 1 | cut -d '/' -f 3 | cut -d'-' -f 1`
	OE_VERSION=`cd "${AREADELTASAT_BASE}/openembedded-core" && git describe --match=yocto* | cut -d '-' -f 2`

#	Image type, version, revision

	IMAGE_VERSION=`echo ${DISTRO_VERSION} | cut -d "-" -f 1`
	if [ "${IMAGE_VERSION}" = "${DISTRO_VERSION}" ]; then
		OE_REVISION=`cd ${AREADELTASAT_BASE} && git rev-parse --short HEAD`
		IMAGE_TYPE='rev'
	else
		OE_REVISION=
...
#	Generate the info file
	...
	printf "displaydistro='areadeltasat'\n" >> ${D}${INFOFILE}
